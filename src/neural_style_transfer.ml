...
let load_pretrained_vgg ~filename ~device =
  ...
  let model = Torch_vision.Vgg.vgg16_layers vs ~max_layer ~batch_norm:false |> Staged.unstage in
...
let apply_style_transfer ~style_img:style_img ~content_img:content_img ~filename:filename = 
